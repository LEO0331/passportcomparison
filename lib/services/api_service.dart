...
class ApiService {
  ...
  Future<Set<String>> fetchVisaFreeCodes(String countryCode) async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/visa-single/$countryCode'));
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        Set<String> codes = {};
        // 將免簽、落地簽、電子簽皆視為可准入 (可依需求細分)
        for (var category in ['visa_free_access', 'visa_on_arrival', 'visa_online']) {
      ...
      _logger.e("Error fetching visa details", error: e, stackTrace: stackTrace);
    }
    return {};
  }
}
